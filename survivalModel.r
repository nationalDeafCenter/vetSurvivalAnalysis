...
full <- function(dat,survSE=TRUE){

  crossTabs <- xtabs(~deaf+drat+recentVet+diss,dat)
...
results <-
  list(
    ...
    severe=full(filter(dat,drat==5),FALSE)
  )

save(results,file='fittedModels/results.RData')
